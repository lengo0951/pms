# System Architecture — PMS VNU-HCM Frontend

**Last Updated**: 2026-04-18
**Version**: 0.1.0
**Project**: PMS VNU-HCM Research Task Management System (Frontend Demo)

## Overview

PMS VNU-HCM Frontend is a Next.js 15 web application implementing dual-role interface (Researcher + Admin) for university research task management. Architecture uses App Router with route groups for modular layouts, shared component library with theme variants, and mock TS data layer. No backend yet; designed for future API integration via environment variables.

## High-Level Architecture

### Pattern Classification
**Primary Pattern**: Route Groups + Shared Layout (Next.js App Router)
**Secondary Patterns**:
- Theme Variant Pattern (CSS variables + conditional styling)
- Mock Repository Pattern (TS data + helper functions)
- Form Controller Pattern (React Hook Form + Zod)
- Component Composition (reusable UI building blocks)

### Design Philosophy
- **Route Group Isolation**: `(auth)`, `(nkh)`, `/admin` → separate layouts without URL pollution
- **Shared Component Library**: Core UI components (Button, Input, Form, Table, etc.) reused across roles
- **Theme Variants**: Single theme token file with CSS variables toggled per role
- **Mock-First Data**: TS objects simulate backend; easy refactor to API calls
- **Client-First Rendering**: Server Components default; Client Components only where state needed
